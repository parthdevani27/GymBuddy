    ...
    <div className="h-full flex flex-col p-4 bg-slate-950">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-white flex items-center gap-2">
            <CalIcon className="text-blue-500"/> Calendar
        </h2>
        <div className="flex items-center gap-4 bg-slate-900 rounded-lg p-1 border border-slate-800">
          ...
          <span className="text-white font-semibold w-32 text-center">{monthNames[month]} {year}</span>
          <button onClick={nextMonth} className="p-2 hover:bg-slate-800 rounded-md text-white"><ChevronRight size={20}/></button>
        </div>
      </div>

      <div className="grid grid-cols-7 gap-2 mb-2 text-center text-slate-500 font-semibold text-sm">
        {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(d => <div key={d}>{d}</div>)}
      </div>

      <div className="grid grid-cols-7 gap-2 auto-rows-fr flex-1 overflow-y-auto">
        ...
        {Array.from({ length: daysInMonth }).map((_, i) => {
          ...
          return (
            <button
              ...
              className={`relative rounded-xl border p-2 flex flex-col items-start justify-between hover:scale-[1.02] transition shadow-sm ${bgColor} ${borderColor} min-h-[80px] overflow-hidden`}
            >
              <span className={`text-sm font-bold ${isToday ? 'text-blue-400' : 'text-slate-300'}`}>{day}</span>
              
              <div className="flex flex-col items-end w-full gap-1">
                 {log?.caloriesBurned !== undefined && (
                     <span className="flex items-center gap-0.5 text-[9px] text-orange-400 font-bold bg-slate-950/50 px-1 rounded truncate max-w-full">
                         <Flame size={8} fill="currentColor" className="flex-shrink-0" /> {Math.round(log.caloriesBurned)} kcal
                     </span>
                 )}
                 {log?.bodyWeight && (
                     <span className="text-[10px] bg-slate-800 text-slate-300 px-1 rounded truncate max-w-full">{log.bodyWeight}kg</span>
                 )}
