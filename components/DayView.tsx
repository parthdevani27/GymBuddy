...
import { Save, Plus, Camera, Bot, Trash2, ArrowLeft, CheckCircle2, Dumbbell, Lightbulb, Loader2, Calculator, BrainCircuit, CalendarClock, Lock, History } from 'lucide-react';
...
export const DayView: React.FC<Props> = ({ dateStr, weeklyPlan, log, onSaveLog, onBack, setHasUnsavedChanges }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 overflow-y-auto">
      {/* Header */}
      <div className="sticky top-0 z-10 bg-slate-900/95 backdrop-blur border-b border-slate-700 p-4 flex items-center justify-between shadow-lg">
            ...
            {currentLog.loggedExercises.map((ex, exIdx) => (
              <div key={ex.id} className={`bg-slate-900 rounded-2xl border border-slate-800 shadow-xl overflow-hidden group ${!isEditable ? 'opacity-90' : ''}`}>
                ...
                <div className="p-5">
                    ...
                    {/* Sets Table Header */}
                    <div className="grid grid-cols-12 gap-2 text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-2 px-1">
                    ...
                    <div className="space-y-2">
                        {ex.setsPerformed.map((set, setIdx) => (
                            <div key={set.id} className="grid grid-cols-12 gap-2 items-center group/set">
                                ...
                                <div className="col-span-4">
                                    <input 
                                        ...
                                        className={`w-full bg-slate-950 border border-slate-700 rounded-lg py-2 text-center text-white font-mono focus:border-blue-500 outline-none transition-colors ${!isEditable ? 'opacity-50' : ''}`}
                                    />
                                </div>
                                <div className="col-span-4">
                                    <input 
                                ...
                                <div className="col-span-2 flex justify-center">
                                    <label className={`cursor-pointer relative ${!isEditable ? 'pointer-events-none opacity-50' : ''}`}>
                                        ...
                                        />
                                        <div className="w-8 h-8 rounded-lg border-2 border-slate-700 bg-slate-900 peer-checked:bg-green-500 peer-checked:border-green-500 transition-all flex items-center justify-center text-slate-900">
                                            <CheckCircle2 size={18} className="opacity-0 peer-checked:opacity-100 transition-opacity" />
                                        </div>
                                    </label>
                                </div>
                                <div className="col-span-1 flex justify-center">
                                    {isEditable && (
                                        <button 
                                            ...
                                            className="text-slate-700 hover:text-red-500 transition-colors opacity-0 group-hover/set:opacity-100"
                                        >
                                            <Trash2 size={16}/>
                                        </button>
                                    )}
